...
Vagrant.configure("2") do |config|
  ...
  # Setting box version
  if my_env[:cf_box_version] then
    config.vm.box_version = my_env[:cf_box_version]
  ...
  config.vm.synced_folder my_env[:cf_app_source_path], "/app/source", id: "source", group: my_env[:cf_http_group], mount_options: ["dmode=775,fmode=775"]

  if my_env[:cf_private_key_path] && my_env[:cf_public_key_path] then
    # Using existed private key and do not generate a key
    config.ssh.insert_key = false
    config.ssh.private_key_path = [my_env[:cf_private_key_path], "~/.vagrant.d/insecure_private_key"]
    # Copy public key to VM
    config.vm.provision "file", source: my_env[:cf_public_key_path], destination: "~/.ssh/authorized_keys"
  end
